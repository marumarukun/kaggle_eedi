...
def preprocess_train(train_df: pl.DataFrame) -> pl.DataFrame:
    ...
    train_correct_answer = (
        train_long.filter(pl.col("CorrectAnswer") == pl.col("AnswerAlphabet"))
        .select(pl.col(["QuestionId", "AnswerText"]))
        .rename({"AnswerText": "CorrectAnswerText"})
    )

    # train_longに結合し、正解のテキスト列を追加
    train_long = train_long.join(train_correct_answer, on="QuestionId", how="left")
